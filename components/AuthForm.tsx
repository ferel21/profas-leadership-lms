...
export function AuthForm({ mode, redirectTo = "/dashboard" }: { mode: "login" | "register"; redirectTo?: string }) {
  ...
  const demo = (email: string) => { const input = document.querySelector<HTMLInputElement>('input[name="email"]'); const pass = document.querySelector<HTMLInputElement>('input[name="password"]'); if (input && pass) { input.value = email; pass.value = "profas123"; } };
  return <form onSubmit={submit} className="auth-form">
    <a href="/api/auth/google" className="btn btn-outline" style={{ display: 'flex', width: '100%', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', marginBottom: '1.5rem', background: 'white' }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', textAlign: 'center', marginBottom: '1.5rem', color: 'var(--muted)', fontSize: '0.85rem' }}>
      ...
      <span style={{ padding: '0 10px' }}>atau dengan email</span>
      <div style={{ flex: 1, borderBottom: '1px solid var(--line)' }}></div>
    </div>
    ...
    <button className="btn btn-primary auth-submit" disabled={loading}>{loading?<LoaderCircle className="spin"/>:<>{mode==="login"?"Masuk ke Dashboard":"Buat Akun Gratis"}<ArrowRight/></>}</button>
    {mode === "login" && <div className="demo-box"><b>Akun demo</b><div><button type="button" onClick={()=>demo("peserta@profas.id")}>Peserta</button><button type="button" onClick={()=>demo("mentor@profas.id")}>Mentor</button><button type="button" onClick={()=>demo("admin@profas.id")}>Admin</button></div><small>Kata sandi semua akun: profas123</small></div>}
  </form>;
}
