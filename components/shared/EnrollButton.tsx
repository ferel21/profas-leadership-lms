...
export function EnrollButton({ courseId, slug, signedIn, enrolled = false }: { courseId: string; slug: string; signedIn: boolean; enrolled?: boolean }) {
  ...
  return <div className="enroll-action-wrap"><button className="btn btn-primary enroll-button" onClick={enroll} disabled={loading} aria-busy={loading}>{loading?<><LoaderCircle className="spin" aria-hidden="true"/><span>Menyiapkan akses…</span></>:<><span>{enrolled?"Lanjutkan Belajar":"Mulai Program"}</span><ArrowRight aria-hidden="true"/></>}</button>{message && <p className="enroll-error" role="alert">{message}</p>}</div>;
}
